...
## GitHub CLI — Account Rule

This machine has two `gh` accounts: `0o-de-lally` (write access) and `lucky-tensor` (read-only).

- **Never run `gh auth switch`** — it permanently changes the active account.
- For any `gh` command that writes to GitHub (issue edit, PR create, issue comment, plan update, etc.), prefix with the inline token:

```bash
GH_TOKEN=$(gh auth token --user 0o-de-lally) gh issue edit ...
GH_TOKEN=$(gh auth token --user 0o-de-lally) gh pr create ...
```

- Read-only commands (`gh issue view`, `gh issue list`, `gh pr status`) do not need the prefix.
