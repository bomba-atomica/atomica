...
**Protocol Advantages:**
- **Simpler Mechanism:** No reserve prices = no reserve price penalty complexity
- **Easier Bootstrapping:** Critical mass in single auction vs. fragmenting across many
- **Lower Coordination Overhead:** One auction to settle vs. continuous coordination
- **Aligned Incentives:** Without reserve prices, all participants want auction to succeed
...
**Implementation:**
- Bids encrypted using drand-based timelock encryption (detailed in [Timelock Encryption for Sealed Bids](timelock-bids.md))
...
As documented in `timelock-bids.md`, this approach is **practical and implementable** using:
...
- No interactive reveal phase (prevents griefing)
...
| **MM Appeal** | Lower (thin volume) | Higher (critical mass) |
| **Bootstrapping** | Hard (many auctions to fill) | Easier (one large auction) |
| **Complexity** | Higher (reserve mechanism) | Simpler |
| **Price Discovery** | Continuous | Batch (daily) |
...
- Let market determine optimal auction timing based on volume
