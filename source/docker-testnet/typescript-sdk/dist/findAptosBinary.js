...
function findAptosBinary() {
    ...
    if (candidates.length === 0) {
        throw new Error(`❌ Aptos CLI binary ('aptos') not found in any expected location.\n\n` +
            `SEARCHED LOCATIONS:\n` +
...
function findInPath(binaryName) {
    ...
    if (result.status === 0 && result.stdout) {
        ...
        for (const path of paths) {
            ...
            if (trimmedPath && (0, fs_1.existsSync)(trimmedPath) && !trimmedPath.includes("node_modules")) {
                return trimmedPath;
            }
        }
    }
