...
async function main() {
  ...
  try {
    ...
    console.log(
      `   Private Key: ${deployerAccount.privateKey.slice(0, 10)}...`,
    );
    ...
    console.log(
      `   Locked balance: ${ethers.formatEther(lockedBalance)} FAKETH`,
    );

    console.log("\n⏳ Step 5/6: Waiting for finality...");
    console.log(
      `   Waiting for blocks after lock transaction (block ${lockBlock})...`,
    );
