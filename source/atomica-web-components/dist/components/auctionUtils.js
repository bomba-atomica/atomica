 ...
 */
export function getUpcomingAuctions(now) {
    ...
    const candidates = AUCTION_WINDOWS.map(({ name, utcHour, utcMinute }) => {
        const d = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), utcHour, utcMinute, 0, 0));
...
//# sourceMappingURL=auctionUtils.js.map