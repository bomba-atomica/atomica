...
export function AuctionBidder() {
    ...
    const handleBid = async () => {
        ...
        try {
            ...
            const pairBcs = pairBcsHex
                ? new Uint8Array((pairBcsHex.replace(/^0x/, "").match(/.{1,2}/g) ?? []).map((b) => parseInt(b, 16)))
                : new Uint8Array(0);
            const collateralLockId = collateralLockIdHex
                ? new Uint8Array((collateralLockIdHex.replace(/^0x/, "").match(/.{1,2}/g) ?? []).map((b) => parseInt(b, 16)))
                : new Uint8Array(32); // placeholder: 32 zero bytes
            // 3. Submit
            setStatus("Please sign the transaction...");
            const pendingTx = await submitBid(account, windowIdBn, pairBcs, u, v, collateralLockId);
    ...
    return (_jsxs("div", { className: "bg-zinc-900 p-6 rounded-lg border border-zinc-800", children: [_jsx("h2", { className: "text-xl font-bold mb-4 text-zinc-300", children: "Buy" }), _jsxs("div", { className: "space-y-4", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-zinc-500 text-sm mb-1", children: "Window ID" }), _jsx("input", { "data-testid": "window-id-input", type: "number", value: windowId, onChange: (e) => setWindowId(e.target.value), className: "w-full bg-zinc-800 text-zinc-200 rounded p-2 border border-zinc-700 focus:outline-none focus:border-zinc-500" })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-zinc-500 text-sm mb-1", children: "Pair BCS (0x hex, optional)" }), _jsx("input", { type: "text", value: pairBcsHex, onChange: (e) => setPairBcsHex(e.target.value), placeholder: "0x... (leave empty for default)", className: "w-full bg-zinc-800 text-zinc-200 rounded p-2 text-xs font-mono border border-zinc-700 focus:outline-none focus:border-zinc-500" })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-zinc-500 text-sm mb-1", children: "Collateral Lock ID (0x hex)" }), _jsx("input", { "data-testid": "collateral-lock-id-input", type: "text", value: collateralLockIdHex, onChange: (e) => setCollateralLockIdHex(e.target.value), className: "w-full bg-zinc-800 text-zinc-200 rounded p-2 text-xs font-mono border border-zinc-700 focus:outline-none focus:border-zinc-500", placeholder: "0x..." })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-zinc-500 text-sm mb-1", children: "Bid Amount (USD)" }), _jsx("input", { "data-testid": "bid-amount-input", type: "number", value: bidAmount, onChange: (e) => setBidAmount(e.target.value), className: "w-full bg-zinc-800 text-zinc-200 rounded p-2 border border-zinc-700 focus:outline-none focus:border-zinc-500" })] }), _jsx("button", { "data-testid": "submit-bid-button", onClick: handleBid, disabled: loading, className: `w-full py-2 rounded font-bold transition-colors ${loading
