 ...
 */
export function usePoolEvents(onEvent) {
    ...
    useEffect(() => {
        ...
        const contract = new ethers.Contract(lockBoxAddress, LOCKBOX_EVENT_ABI, provider);
        ...
        contract.on("TokensLocked", listener).catch((err) => {
            console.warn("[usePoolEvents] Failed to subscribe to TokensLocked events:", err);
        });
        // Cleanup: remove the listener when the component unmounts.
        return () => {
            contract.off("TokensLocked", listener).catch((err) => {
                console.warn("[usePoolEvents] Failed to remove TokensLocked listener:", err);
            });
        };
...
//# sourceMappingURL=usePoolEvents.js.map