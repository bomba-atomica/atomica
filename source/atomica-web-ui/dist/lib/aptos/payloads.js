...
export async function requestAPT(ethAddress) {
    ...
    if (!res.ok) {
        ...
        console.error(`Funding API Failed: ${res.status} ${res.statusText} - ${text}`);
...
export async function areCoreContractsDeployed() {
 ...
 */
export function getRegisterLockPayload(proof) {
    return {
 ...
 */
export function getMintFakeEthPayload(lockId) {
    return {
