 ...
 */
export function getLockKey(userAddress, tokenAddress) {
    ...
    const compositeKey = ethers.keccak256(ethers.solidityPacked(["address", "address"], [user, token]));
    return compositeKey;
}
/**
 ...
 */
export function calculateLockedBalanceStorageKey(userAddress, tokenAddress, slot = 0) {
    ...
    const storageKey = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(["bytes32", "uint256"], [compositeKey, slot]));
    return storageKey;
}
/**
 ...
 * @returns Storage key as hex string
 */
export function calculateUnlockTimeStorageKey(userAddress, tokenAddress) {
 ...
 */
export async function verifyStorageKeyCalculation(contract, userAddress, tokenAddress) {
    try {
        // Get key from contract's helper function
        const onChainKey = await contract.calculateStorageKey(userAddress, tokenAddress);
        // Calculate key off-chain
        const offChainKey = calculateLockedBalanceStorageKey(userAddress, tokenAddress);
 ...
 */
export function calculateBatchStorageKeys(users, tokens) {
    const keys = [];
