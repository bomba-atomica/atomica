...
describe.skip.sequential("E2E: Timelock Round Trip", () => {
  ...
  it("should complete encryption and decryption cycle", async () => {
    ...
    const event = (txnResponse as any).events.find(
      (e: any) => e.type === "0x1::ibe_config::TimelockRegistrationEvent",
    );

    expect(event).toBeDefined();
    const timelockId = BigInt(event!.data.timelock_id);
    console.log(
      `Registered timelock ${timelockId} with deadline ${deadlineUs}`,
    );
