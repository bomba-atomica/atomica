...
describe("SECP256k1 Address Derivation: Ethereum vs Aptos", () => {
  ...
  describe("Ethereum Address Derivation (Keccak-256)", () => {
    it("should derive Ethereum address from private key using ethers.js", () => {
      ...
      console.log(
        `Address length: ${ethAddress.length} characters (0x + 40 hex = 20 bytes)`,
      );
      ...
      console.log(
        `Public key length: ${publicKey.length} characters (0x + 130 hex = 65 bytes uncompressed)`,
      );

      // Step 4: Explain the derivation process
      console.log(`\nStep 4: Ethereum Address Derivation Process:`);
      console.log(
        `1. Private key (32 bytes) → SECP256k1 curve → Public key (65 bytes uncompressed)`,
      );
    ...
    it("should demonstrate Keccak-256 hashing step-by-step", () => {
      ...
      console.log(
        `   - First byte: 0x${publicKeyBytes[0].toString(16).padStart(2, "0")} (the 0x04 prefix)`,
      );

      // For Ethereum, we hash the 64 bytes (without the 0x04 prefix)
      const publicKeyWithoutPrefix = publicKeyBytes.slice(1);
      console.log(
        `   - Without prefix: ${publicKeyWithoutPrefix.length} bytes`,
      );
      ...
      console.log(
        `   - Hash length: ${keccakHash.length - 2} hex chars = 32 bytes`,
      );
  ...
  describe("Aptos Address Derivation (SHA3-256)", () => {
    it("should derive Aptos address from same private key using Aptos SDK", () => {
      ...
      console.log(
        `Address length: ${aptosAddress.length} characters (0x + 64 hex = 32 bytes)`,
      );
  ...
  describe("Cross-Chain Compatibility: Same Key, Different Addresses", () => {
    ...
    it("should demonstrate the UX problem for Ethereum users", () => {
      ...
      console.log(
        `  She uses the SAME private key: ${testPrivateKeyHex.slice(0, 20)}...`,
      );
      console.log(
        `  Aptos shows her address: ${aptosAccount.accountAddress.toString()}\n`,
      );
  ...
  describe("Public Key Compatibility", () => {
    it("should prove same private key produces SAME public key on both chains", () => {
      ...
      console.log(
        `  Length: ${ethPublicKeyBytes.length} bytes (65 bytes uncompressed)`,
      );
      ...
      console.log(
        `  Public key bytes length: ${aptosPublicKeyBytes.length} bytes`,
      );
      console.log(
        `  First byte: 0x${aptosPublicKeyBytes[0].toString(16).padStart(2, "0")}\n`,
      );
    ...
    it("should verify SECP256k1 curve parameters are identical", () => {
      ...
      console.log(`  Private key: ${testPrivateKeyHex}`);
      console.log(
        `  → Ethereum public key length: ${ethers.getBytes(ethWallet.signingKey.publicKey).length} bytes`,
      );
      console.log(
        `  → Aptos public key length: ${aptosAccount.publicKey.publicKey.toUint8Array().length} bytes`,
      );
  ...
  describe("Signature Compatibility", () => {
    it("should prove signatures are compatible (same message, same signature)", () => {
      ...
      console.log(
        `  Signature bytes: ${aptosSignature.signature.toUint8Array().length} bytes (64 bytes: r + s)\n`,
      );
      ...
      console.log(
        `Signature verification on Aptos: ${isValid ? "✅ Valid" : "❌ Invalid"}\n`,
      );
  ...
  describe("Address Derivation Summary", () => {
    it("should provide comprehensive comparison table", () => {
      ...
      console.log(
        "│   Ethereum: Keccak-256 (pre-NIST SHA-3)                         │",
      );
      console.log(
      ...
      console.log(
        "│ ADDRESS LENGTH (Different)                                      │",
      );
      console.log(
        "│   Ethereum: 20 bytes (40 hex) - last 20 bytes of hash           │",
      );
      console.log(
        "│   Aptos:    32 bytes (64 hex) - full hash                       │",
      );
      console.log(
        "│                                                                 │",
      );
      ...
      console.log(
        "│   Both: SECP256k1 ECDSA, 64 bytes (r + s)                       │",
      );
      console.log(
        "│   Recovery: 1 byte (v parameter)                                │",
      );
      console.log(
        "│                                                                 │",
      );
