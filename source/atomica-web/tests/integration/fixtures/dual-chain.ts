/**
 * Browser-test-safe dual-chain fixture for integration test suites.
 *
 ...
 *
 * Under the hood this delegates to the setupDualChainTestnet browser command
 * which runs in Node.js (required for Docker / child_process access) and
 * returns JSON-serialisable connection info that the browser can use.
 *
 * @example
 * ```ts
 ...
 * fund the seller and bidder Ethereum addresses.
 *
...
export async function setupIntegrationFixture(): Promise<IntegrationFixture> {
  ...
  const info: DualChainTestnetInfo =
    await commands.setupDualChainTestnet();
  ...
  console.log(
    `[IntegrationFixture] Seller: ${fixture.eth.seller.address}`,
  );
  console.log(
    `[IntegrationFixture] Bidder: ${fixture.eth.bidder.address}`,
  );
  console.log(
    `[IntegrationFixture] LockBox: ${fixture.eth.contracts.lockBox}`,
  );
  console.log(
...
/**
 * Tear down both testnets started by setupIntegrationFixture().
 *
