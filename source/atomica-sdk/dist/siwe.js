...
export function constructSIWEMessage(domain, address, statement, uri, version, chainId, nonce, issuedAt) {
 ...
 */
export async function getDerivedAddress(ethAddress, domain) {
 ...
 */
export function calculateAbstractDigest(signingMessage) {
 ...
 */
export function serializeSIWEAbstractSignature(scheme, issuedAt, signature) {
    const serializer = new Serializer();
 ...
 */
export function serializeSIWEAbstractPublicKey(ethAddressBytes, domain) {
    const serializer = new Serializer();
...
export class SIWEAccountAuthenticator extends AccountAuthenticator {
    ...
    abstractSignature;
    accountIdentity;
    constructor(digest, abstractSignature, accountIdentity) {
        super();
        this.digest = digest;
