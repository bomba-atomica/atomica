...
import { getDerivedAddress, calculateAbstractDigest, constructSIWEMessage, serializeSIWEAbstractSignature, serializeSIWEAbstractPublicKey, SIWEAccountAuthenticator, } from "./siwe.js";
function sha3(bytes) {
    return sha3_256(bytes);
}
export async function prepareNativeTransaction(aptos, ethAddress, payload) {
    if (!window.ethereum)
    ...
    if (ledgerInfo.chain_id === 1)
        networkName = "mainnet";
    else if (ledgerInfo.chain_id === 2)
        networkName = "testnet";
    else if (ledgerInfo.chain_id === 4)
        networkName = "local";
    // Extract Entry Function Name
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const entryFunction = payload.function;
    if (!entryFunction)
    ...
    const siwe = constructSIWEMessage(domain, checksummedAddress, statement, window.location.origin, "1", ledgerInfo.chain_id, digestHex, issuedAt);
    // 5. Sign
    ...
    const abstractSignature = serializeSIWEAbstractSignature(scheme, issuedAt, signatureBytes);
    // Build the SIWE abstract public key (ethAddress, domain)
    const accountIdentity = serializeSIWEAbstractPublicKey(ethAddressBytes, domain);
    // Create account authenticator using SDK enums
    const auth = new SIWEAccountAuthenticator(digest, abstractSignature, accountIdentity);
    return {
        transaction,
        auth,
        senderAddress,
...
export async function simulateNativeTransaction(aptos, preparedTx) {
    try {
...
export async function submitPreparedTransaction(aptos, preparedTx) {
    ...
    try {
        ...
        throw new Error(`Transaction Submission verification failed: ${errorMessage}\nDebug State: ${JSON.stringify(debugState, null, 2)}`);
    }
}
export async function submitNativeTransaction(aptos, ethAddress, payload) {
    // Overload: Check if payload is actually a PreparedTransaction
    if ("auth" in payload && "transaction" in payload) {
        return submitPreparedTransaction(aptos, payload);
    }
    // Default flow: Prepare -> Simulate -> Submit
    const prepared = await prepareNativeTransaction(aptos, ethAddress, payload);
    // Backward compatibility: Simulate and log, but don't block unless error throws
    try {
        await simulateNativeTransaction(aptos, prepared);
    }
    catch {
        console.error("Simulation error caught in wrapper, proceeding to submit...");
    }
    return await submitPreparedTransaction(aptos, prepared);
}
