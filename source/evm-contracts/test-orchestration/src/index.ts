...
export class TestOrchestrator {
  ...
  async deployContracts(): Promise<void> {
    ...
    await this.runCommand("forge", ["script", "script/Deploy.s.sol", "--rpc-url", rpcUrl, "--broadcast"], {
  ...
  async runUnitTests(): Promise<void> {
    ...
    const result = await this.runCommand("forge", ["test", "--match-path", "test/unit/*.sol", "--rpc-url", rpcUrl, "--summary"], {
      cwd: this.config.contractsDir,
      env,
    });
  ...
  async runIntegrationTests(): Promise<void> {
    ...
    const result = await this.runCommand("forge", ["test", "--match-path", "test/integration/*.sol", "--rpc-url", rpcUrl, "--summary"], {
      cwd: this.config.contractsDir,
      env,
    });
  ...
  async runE2ETests(): Promise<void> {
    ...
    const result = await this.runCommand("forge", ["test", "--match-path", "test/e2e/*.sol", "--rpc-url", rpcUrl, "--summary"], {
      cwd: this.config.contractsDir,
      env,
    });
  ...
  async runSmokeTests(): Promise<void> {
    ...
    const result = await this.runCommand("forge", ["test", "--match-test", "testDeposit|testWithdraw", "--match-contract", "DepositBoxTest", "--rpc-url", rpcUrl], {
      cwd: this.config.contractsDir,
      env,
    });
  ...
  private async getDeployedContracts(rpcUrl: string): Promise<Record<string, string>> {
    ...
    const result = await this.runCommand("forge", ["script", "--dry-run", "--json", "script/Deploy.s.sol", "--rpc-url", rpcUrl], {
      cwd: this.config.contractsDir,
    ...
    const deploymentsPath = pathResolve(this.config.contractsDir, "broadcast/Deploy.s.sol/latest/run-latest.json");
   ...
   * Run a shell command
   */
  private async runCommand(command: string, args: string[], options: { cwd?: string; env?: Record<string, string> } = {}): Promise<string> {
 ...
 */
export async function getDeploymentInfo(rpcUrl: string): Promise<Record<string, string>> {
