...
export function initRemoteLogger() {
  ...
  if (
    (window as { __remoteLoggerInitialized?: boolean })
      .__remoteLoggerInitialized
  )
    return;
  (
    window as { __remoteLoggerInitialized?: boolean }
  ).__remoteLoggerInitialized = true;
  ...
  console.table = (...args) => {
    originalLog.apply(console, args);
    send("info", ["[TABLE]", ...args]);
  };
  console.trace = (...args) => {
    originalLog.apply(console, args);
    send("debug", ["[TRACE]", ...args]);
  };
  console.dir = (...args) => {
    originalLog.apply(console, args);
    send("debug", ["[DIR]", ...args]);
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      if (target instanceof HTMLElement) {
        ...
        if (src) {
          send("error", [
            `[Resource Error] Failed to load ${target.tagName}: ${src}`,
          ]);
          return;
        }
      }
    ...
    true,
  ); // useCapture = true to catch non-bubbling resource errors
  ...
  // @ts-expect-error - Bun's fetch has different type signature than browser
            ...
            const url =
              typeof input === "string"
                ...
                : input instanceof URL
                  ...
                  : input.url;
            send("error", [
              `[Network Error] ${status} ${response.statusText} on ${url}`,
              bodySnippet,
            ]);
          })
          .catch(() => {
      ...
      const url =
        typeof input === "string"
          ...
          : input instanceof URL
            ...
            : input.url;
      send("error", [
        `[Network Request Failed] ${url}: ${e instanceof Error ? e.message : String(e)}`,
      ]);
      throw e;
    }
  };
